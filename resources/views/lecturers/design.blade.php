                        ...
                        <div class="modal-dialog modal-xl">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="updatePaperModal">{{ $student->first_name}} {{ $student->last_name}} Design</h1>
                                ...
                                <div class="modal-body" style="padding: 2rem 1rem;">
                                    @if($student->design)
                                        <div class="d-flex flex-column gap-4">
                                            <div>
                                                @if($student->design->status === 'Approved')
                                                    <span class="badge bg-success py-2 px-3"><i class="fas fa-check-circle" style="margin-right: .3rem;"></i> {{ $student->design->status }}</span>
                                                @elseif($student->design->status === 'Rejected')
                                                    <span class="badge bg-danger py-2 px-3"><i class="fas fa-close" style="margin-right: .3rem;"></i> {{ $student->design->status }}</span>
                                                @endif
                                            </div>
                                            <div class="form-group">
                                                <label for="" class="mb-3" style="font-weight: 400;">Title</label>
                                                <input type="text" name="title" class="form-control custom-input" value="{{ $student->design->title }}" required readonly />
                                            </div>
                                            <div class="form-group">
                                                <label for="" class="mb-3" style="font-weight: 400;">Description</label>
                                                <textarea name="description" class="form-control custom-input" rows="4" required readonly>{{ $student->design->description }}</textarea>
                                            </div>
                                            <div class="d-flex flex-column gap-2">
                                                <img src="{{ asset('assets/images/pdf.png') }}" style="width: 40px; margn-bottom: 0;" />
                                                <a href="/storage/{{ $student->design->file }}" target="_blank" class="text-decoration-underline mt-0">Documentation</a>
                                            </div>
                                        </div>

                                        @if($student->design->status === 'Pending')                                       
                                            <form action="/mark-design/{{ $student->design->id }}" method="post">
                                                @csrf
