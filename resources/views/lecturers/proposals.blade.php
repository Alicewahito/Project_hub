                        ...
                        <div class="modal-dialog modal-xl">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="updatePaperModal">{{ $student->first_name}} {{ $student->last_name}} Proposal</h1>
                                ...
                                <div class="modal-body" style="padding: 2rem 1rem;">
                                    @if($student->proposal)
                                        <div class="d-flex flex-column gap-4">
                                            <div>
                                                @if($student->proposal->status === 'Approved')
                                                    <span class="badge bg-success py-2 px-3"><i class="fas fa-check-circle" style="margin-right: .3rem;"></i> {{ $student->proposal->status }}</span>
                                                @elseif($student->proposal->status === 'Rejected')
                                                    <span class="badge bg-danger py-2 px-3"><i class="fas fa-close" style="margin-right: .3rem;"></i> {{ $student->proposal->status }}</span>
                                                @endif
                                            </div>
                                            <div class="form-group">
                                                <label for="" class="mb-3" style="font-weight: 400;">Title</label>
                                                <input type="text" name="title" class="form-control custom-input" value="{{ $student->proposal->title }}" required readonly />
                                            </div>
                                            <div class="form-group">
                                                <label for="" class="mb-3" style="font-weight: 400;">Description</label>
                                                <textarea name="description" class="form-control custom-input" rows="4" required readonly>{{ $student->proposal->description }}</textarea>
                                            </div>
                                        ...
                                        @if($student->proposal === 'Pending')
                                       
                                            <form action="/mark-proposal/{{ $student->proposal->id }}" method="post">
                                                @csrf
