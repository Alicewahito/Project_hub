                        ...
                        <div class="modal-dialog modal-xl">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="updatePaperModal">{{ $student->first_name}} {{ $student->last_name}} Implementation</h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body" style="padding: 2rem 1rem;">
                                    @if($student->implementation)
                                        <div class="d-flex flex-column gap-4">
                                            <div>
                                                @if($student->implementation->status === 'Approved')
                                                    <span class="badge bg-success py-2 px-3"><i class="fas fa-check-circle" style="margin-right: .3rem;"></i> {{ $student->implementation->status }}</span>
                                                @elseif($student->implementation->status === 'Rejected')
                                                    <span class="badge bg-danger py-2 px-3"><i class="fas fa-close" style="margin-right: .3rem;"></i> {{ $student->implementation->status }}</span>
                                                @endif
                                            </div>
                                            <div class="d-flex align-items-center gap-5">
                                                <div class="form-group w-100">
                                                    <label for="" class="mb-3" style="font-weight: 400;">Title</label>
                                                    <input type="text" name="title" class="form-control custom-input" value="{{ $student->implementation->title }}" required readonly />
                                                </div>
                                                <div class="form-group w-100">
                                                    <label for="" class="mb-3" style="font-weight: 400;">Github Repo</label>
                                                    <input type="text" name="title" class="form-control custom-input" value="{{ $student->implementation->github_repo }}" required readonly />
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label for="" class="mb-3" style="font-weight: 400;">Description</label>
                                                <textarea name="description" class="form-control custom-input" rows="4" required readonly>{{ $student->implementation->description }}</textarea>
                                            </div>
                                        </div>

                                        <div class="d-flex flex-column gap-2">
                                            <img src="{{ asset('assets/images/pdf.png') }}" style="width: 40px; margn-bottom: 0;" />
                                            <a href="/storage/{{ $student->implementation->file }}" target="_blank" class="text-decoration-underline mt-0">Documentation</a>
                                        </div>

                                        @if($student->implementation->status === 'Pending')                                       
                                            <form action="/mark-design/{{ $student->implementation->id }}" method="post">
                                                @csrf
